...
class HttpHandler(ProtocolHandler):
    ...
    def process_packet(self, packet):
        ...
        if self.src_ip and self.dst_ip:
            print(f"HTTP Packet detected: {self.src_ip} -> {self.dst_ip}, Payload: {packet[Raw].load[:50]}")
...
class DnsHandler(ProtocolHandler):
    ...
    def process_packet(self, packet):
        ...
        if self.src_ip and self.dst_ip:
            ...
            print(f"DNS Packet detected: {self.src_ip} -> {self.dst_ip}, Query: {query}")
...
class FTPHandler(ProtocolHandler):
    ...
    def detect(self, packet):
        ...
        return packet.haslayer(TCP) and (packet[TCP].dport == 21 or packet[TCP].dport == 20)
...
class SMTPHandler(ProtocolHandler):
    ...
    def detect(self, packet):
        ...
        return packet.haslayer(TCP) and (packet[TCP].dport in [25, 465, 587] or packet[TCP].sport in [25, 465, 587])
    def process_packet(self, packet):
        self.extract_ip(packet)
        if self.src_ip and self.dst_ip:
