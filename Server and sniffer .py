...
def tcp_server():
    ...
    def handle_client(client_socket, forward_to_host, forward_to_port):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as forward_socket:
            ...
            client_socket.send(
                response
            )  # Froward response to client (We are working as proxy)
        client_socket.close()  # Close Client socket

    sock = socket.socket(
        socket.AF_INET, socket.SOCK_STREAM
    )  # Init stream socket , AF_INENT - Internet Protocol , SOCK_STREAM - We want a stream-based protocol because of TCP.
    ...
    while True:
        ...
        client_thread = threading.Thread(
            target=handle_client, args=(client_socket, "example.com", 80)
        )  # Using thread, we call handle_client function.
        client_thread.start()
